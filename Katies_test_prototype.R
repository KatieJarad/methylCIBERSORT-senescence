...
# write.csv(results, "/home/katiej/methylcibersort_test/ciberesults.csv")
##################
...
library(dplyr)
library(ggplot2)
temp.plot <- ggplot(data=pan.pheno, aes(x=Tumour_Type, y=onc_senes)) +
  geom_boxplot()
...
family.cols <- c("Control"  = "grey",
                 "Embryonal" = "red",
                 "Ependymal" = "green",
                 "Glio-neuronal" = "blue", "Glioblastoma"  "Glioma-IDH"    "Glioma-other" 
                 "Melanocytic"   "Mesenchymal"   "Nerve"         "Pineal"        "PLASMA"        "Plexus"        "Sella" )
...
cibersort_near.plot <-ggplot(combined.df, aes(x=Tumour_Type, y=Near_Senescent_BJ_fibroblast_cells, fill=Tumour_Type)) +
  geom_boxplot(outlier.color="black", outlier.size=0.5, outlier.alpha = 0.75) +
  theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
print(cibersort_near.plot)
ggsave(cibersort_onc.plot, file = "./cibersort_near_ses_plot.png", width =  20, height = 10, units = "cm")

cibersort_fibro.plot <-ggplot(combined.df, aes(x=Tumour_Type, y=Senescent_BJ_fibroblast_cells, fill=Tumour_Type)) +
  geom_boxplot(outlier.color="black", outlier.size=0.5, outlier.alpha = 0.75) +
  theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
print(cibersort_fibro.plot)
ggsave(cibersort_onc.plot, file = "./cibersort_ses_fibro_plot.png", width =  20, height = 10, units = "cm")

cibersort_onc.plot <-ggplot(PanPhenoResultsOrdered, aes(x=Tumour_Type, y=Oncogene_Induced_Senescent, fill=Tumour_Type)) +
  geom_boxplot(outlier.color="black", outlier.size=0.5, outlier.alpha = 0.75) +
  theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
print(cibersort_onc.plot)
...
VennDiagram::venn.diagram(
  x = gene.list[c("SenMayo","Avelar")],
  category.names = c("SenMayo","Avelar"),
  filename = 'venn1.png', 
  imagetype = "png", 
  disable.logging = T)
...
pheatmap(gene.freq, cluster_rows = F)
...
pheatmap(normalised.matrix.of.overlaps, cluster_rows = F, cluster_cols = F)
...
temp.plot
ggsave(temp.plot, file = "./temp.png", width =  20, height = 10, units = "cm")
